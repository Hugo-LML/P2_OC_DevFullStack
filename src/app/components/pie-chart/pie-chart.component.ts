...
export class PieChartComponent implements OnInit, OnDestroy {
  ...
  defaultView: [number, number] = this.chartsService.getChartView(
    'PIE_CHART_DEFAULT_VIEW'
  );
  ...
  onSelect(event: { name: string; value: number; label: string }) {
    ...
    if (selectedOlympic)
      this.router.navigateByUrl(`olympics/${selectedOlympic.id}`);
  }
  ...
  onResize(): void {
    this.view =
      window.innerWidth < 768
        ? this.chartsService.getChartView('CHART_MOBILE_VIEW')
        : this.defaultView;
  }
